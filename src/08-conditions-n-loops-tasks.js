...
function isCreditCardNumber(ccn) {
  ...
  const newStr = str
    .map((el, index) => (index % 2 !== 0 ? (el * 2).toString() : el))
    .map((el) => (el.length !== 1 ? el.split('') : el)).flat(1).reduce((a, b) => Number(a) + Number(b), 0);
  return newStr % 10 === 0;
}
...
function isBracketsBalanced(/* str */) {
  ...
  //   }
  // }
  // if (result['['] !== result[']'] || arr.indexOf(']') < arr.indexOf('[')) return false;
...
function evaluateTicTacToePosition(position) {
  ...
  if (position[2][0] === position[2][1] && position[2][0] === position[2][1]) {
